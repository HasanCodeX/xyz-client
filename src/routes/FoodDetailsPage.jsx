...
//   useEffect(() => {
//     // Fetch food details including notes
//     const fetchFoodDetails = async () => {
//       try {
//         const res = await fetch(`http://localhost:5000/foods/${id}`);
//         const data = await res.json();
//         setFood(data);
//         setNotes(data.notes || []); // assuming backend returns notes array on food
//       } catch (err) {
...
//   // Check if user added this food (assuming food has addedBy: email or uid)
//   const userCanAddNote = user && food && user.email === food.addedBy;
...
//   if (!user?.email || !food?.userEmail ) return <div className="text-center mt-12">Loading...</div>;
...
//           <strong>Expiration Countdown:</strong>{" "}
//           <span
//             className={
//               calculateCountdown(food.expiryDate) === "Expired"
//                 ? "text-red-600 dark:text-red-400 font-semibold"
//                 : "text-green-700 dark:text-green-400 font-semibold"
//             }
//           >
//             {calculateCountdown(food.expiryDate) || "No expiry date set"}
//           </span>
//         </p>
...
//                 className="border border-gray-300 dark:border-zinc-700 rounded p-3 bg-gray-50 dark:bg-zinc-800"
//               >
...
//           Add Note Form
//           <textarea
//             className="w-full p-3 mb-3 rounded border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-gray-900 dark:text-gray-100 resize-none"
//             rows={4}
//             placeholder={
//               userCanAddNote
//                 ? "You cannot add notes for this food."
...
//             disabled={user?.email === food?.userEmail? false : true}
...
const FoodDetailsPage = () => {
  ...
  const calculateCountdown = (expiryDate) => {
    if (!expiryDate) return { text: "No expiry date set", className: "text-gray-500 dark:text-gray-400" };
    
    const now = new Date();
    const expiry = new Date(expiryDate);
    const diff = expiry - now;
    
    if (diff <= 0) return { text: "Expired", className: "text-red-600 dark:text-red-400 font-semibold" };
  ...
  const countdown = calculateCountdown(food.expiryDate);
